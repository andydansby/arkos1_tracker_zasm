...
;*** Read Special Track ********************************************************
;test #9915	/ +81	#C081
_atVAR_WaitSpecialTrack:
	ld a,1
	dec a
	jr nz,_atSpecialTrackWait

;test #991B	/ +87	#C087
_atVAR_SpecialTrackPtr:
	ld hl,0
	ld a,(hl)
	inc hl
...
;test #99F4	/ + 160	#C160
_atTrack2LoadInstrument:
	; New Instrument. We have to get its new address, and Speed.
	ld l,b					; H is already set to 0 before.
	add hl,hl

;test #99F6	/ + 162	#C162
_atVAR_Track2InstrumentLUT:
	ld bc,0
	add hl,bc
	ld a,(hl)	; Get Instrument address.
	inc hl
...
;test #9A61	/ + 1CD	#C1CD
_atTrack3LoadInstrument:
	;New Instrument. We have to get its new address, and Speed.
	ld l,b	;H is already set to 0 before.
	add hl,hl

;test #9A63	/ + 1CF	#C1CF
_atVAR_Track3InstrumentLUT:
	ld bc,0
	add hl,bc
	ld a,(hl)	;Get Instrument address.
	inc hl
	...
	; Poke &FE as previous R13 value
	inc hl
	ld (_atVAR_Track3InstAddr + 1),hl
	; When using the Instrument again, no need to give the Speed, it is skipped.
...
;test #9AA7	/ + 213	#C213
_atVAR_Track3InstrumentPtr:
	ld hl,0
	call _atPlaySound


;test #9AAD	/ + 219	#C219
_atVAR_Track3InstSpeedCtr:
	ld a,1
	dec a
...
;;attention THIS SECTOIN IS OPTIONAL
	; Play Sound Effects on Track 3 (compiled only if SFX support is enabled)
if atCNF_SFX


_atVAR_SfxTrack3_pitch:
	ld de,0
	exx

_atVAR_SfxTrack3_instrument:
...
_atVAR_SfxTrack3_instrumentSpeedCtr:
	ld a,1
	dec a
...
_atVAR_SfxTrack3_instrumentSpeed:
	ld a,6
...
;test #9AD9	/ + 245	#C245
_atVAR_Track2InstrumentPtr:
	ld hl,0
	call _atPlaySound

;test #9ADF	/ + 24B	#C24B
_atVAR_Track2InstSpeedCtr:
	ld a,1
	dec a
...
_atVAR_SfxTrack2_pitch:
	ld de,0
	exx

_atVAR_SfxTrack2_instrument:
	ld hl,0	; If 0, no sound effect.
	ld a,l
	or h
	jr z,_atNoSfxTrack2
...
_atVAR_SfxTrack2_instrumentSpeedCtr:
	ld a,1
	dec a
...
_atVAR_SfxTrack2_instrumentSpeed:
	ld a,6
...
;test #9B0E	/ + 27A	#C27A
_atVAR_Track1InstrumentPtr:
	ld hl,0
	call _atPlaySound

;test #9B14	/ + 280	#C280
_atVAR_Track1InstSpeedCtr:
	ld a,1
	dec a
...
_atVAR_SfxTrack1_pitch:
	ld de,0
	exx


_atVAR_SfxTrack1_instrument:
	ld hl,0	;If 0, no sound effect.
	ld a,l
	or h
	jr z,_atNoSfxTrack1
...
_atVAR_SfxTrack1_volume:
	;equ $+2	;attention error


_atVAR_SfxTrack1_note:
	;equ $+1	;attention error
	ld de,0	;D=Inverted Volume E=Note
...
_atVAR_SfxTrack1_instrumentSpeedCtr:
	ld a,1
	dec a
...
_atVAR_SfxTrack1_instrumentSpeed:
	ld a,6
...
_atVAR_Reg5:	defb 0
_atVAR_Reg6:	defb 0
_atVAR_Reg8:	defb 0		;+7

; Digidrum ID triggered by the song (0=no digidrum)
_atVAR_digidrum:	defb 0

_atVAR_Reg9:	defb 0		;+9
				defb 0
_atVAR_Reg10:	defb 0		;+11
_atVAR_Reg11:	defb 0
_atVAR_Reg12:	defb 0
_atVAR_Reg13:	defb 0
...
; test #9C13
	...
	ld a,(_atVAR_Track1InstSpeedCtr + 1)
...
if atCNF_ZXPT
	...
	defw 26,25,23,22,21,20,18,17,16,16,15,14
...
atStop:
	...
	djnz $-2
	ld a,%00111111
	; Disable Tone and Noise on all channels
	jp _atAY3Update
